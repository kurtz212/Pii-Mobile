...
const TYPES_VEHICULES = ["Moto", "Voiture", "Tricycle", "Camion léger", "Camion lourd"];

export function CreateAgenceLivraisonScreen() {
  ...
  const canSubmit =
    name.trim().length > 0 &&
    vehicules.length > 0 &&
    zoneCouverture.trim().length > 0 &&
    affiliationCode.trim().length > 0 &&
    !loading;

  function toggleVehicule(v: string) {
    setVehicules((prev) => (prev.includes(v) ? prev.filter((x) => x !== v) : [...prev, v]));
  }
  ...
  async function handleCreate() {
    ...
    try {
      await createEspace({
        type: "agence_livraison" as any,
        name: name.trim(),
        description: description.trim() || undefined,
        location: zoneCouverture.trim(),
        details: { vehicleTypes: vehicules, coverageZone: zoneCouverture.trim() },
  ...
  return (
    <SafeAreaView style={styles.container} edges={["top"]}>
      <View style={styles.headerRow}>
        ...
        <Text style={styles.headerTitle}>Nouvelle agence de livraison</Text>
        <View style={{ width: 20 }} />
      </View>
      ...
      <ScrollView contentContainerStyle={styles.content}>
        ...
        <TextInput
          style={styles.input}
          placeholder="Ex. Rapido Transport"
          placeholderTextColor={colors.textMuted}
          value={name}
          onChangeText={setName}
        />

        <Text style={styles.label}>Types de véhicules disponibles</Text>
        <View style={styles.categoryWrap}>
          {TYPES_VEHICULES.map((v) => {
            const active = vehicules.includes(v);
            return (
              <Pressable
                key={v}
                style={[styles.categoryChip, active && styles.categoryChipActive]}
                onPress={() => toggleVehicule(v)}
              >
        ...
        <TextInput
          ...
          onChangeText={setZoneCouverture}
        />

        <Text style={styles.label}>Description</Text>
        <TextInput
          style={[styles.input, styles.textarea]}
          placeholder="Présente ton agence, tes délais moyens de livraison..."
          placeholderTextColor={colors.textMuted}
          value={description}
        ...
        <View style={styles.infoNote}>
          ...
          <Text style={styles.infoText}>
            Une fois l'agence créée, tu pourras inviter des livreurs à rejoindre ton équipe et
            gérer leurs gains depuis l'espace agence.
          </Text>
        </View>
        ...
        <View style={styles.subscriptionNote}>
          ...
          <Text style={styles.subscriptionText}>
            La création d'une agence de livraison active un abonnement compte pro. Le détail des
        ...
        <Pressable
          ...
          disabled={!canSubmit}
        >
          <Text style={styles.submitButtonText}>{loading ? "Création..." : "Créer mon agence"}</Text>
        </Pressable>
      </ScrollView>
    </SafeAreaView>
  );
