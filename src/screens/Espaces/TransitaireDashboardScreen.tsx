...
export function TransitaireDashboardScreen() {
  ...
  const zonesCouvertes = (espace.details?.zonesCouvertes as string) ?? espace.location;
        ...
        <View style={styles.infoCard}>
          {zonesCouvertes && (
            <View style={styles.infoRow}>
              ...
              <Text style={styles.infoText}>{zonesCouvertes}</Text>
            </View>
          )}
          <View style={styles.infoRow}>
            <Ionicons
            ...
            return (
              <View key={req.id} style={styles.quoteRequestCard}>
                <Text style={styles.quoteRequestRoute}>
                  {d.containerSize} · {d.containerPosition} → {d.destinationZone}
                </Text>
                {d.containerCapacity && (
                  <Text style={styles.quoteRequestDetail}>{d.containerCapacity}</Text>
                )}
                {d.reference && (
...
const styles = StyleSheet.create({
  ...
  quoteRequestReference: { fontSize: 11, color: colors.textMuted, marginTop: 2, fontStyle: "italic" },
